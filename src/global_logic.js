...
module.exports.isCosmos = function(arm) {
  var isCos = false;
  if (
    skilltypes[arm.skill1] != undefined &&
    skilltypes[arm.skill1].type == "cosmosArm"
  ) {
    isCos = true;
  } else if (
    skilltypes[arm.skill2] != undefined &&
    skilltypes[arm.skill2].type == "cosmosArm"
  ) {
    isCos = true;
  }
...
module.exports.calcCombinations = function(arml) {
  ...
  for (var i = 0; i < arml.length; i++) {
    var temp = [];
    var numMin =
      arml[i].considerNumberMin != undefined
        ? parseInt(arml[i].considerNumberMin)
        : 0;
    var numMax =
      arml[i].considerNumberMax != undefined
        ? parseInt(arml[i].considerNumberMax)
        : 1;
...
module.exports.makeSummonHeaderString = function(summon, locale) {
  ...
  if (summon.selfSummonType == "odin") {
    summonHeader +=
      ...
      summon.selfSummonAmount +
      intl.translate("キャラ攻", locale) +
      summon.selfSummonAmount2;
  } else {
    summonHeader +=
      intl.translate(summonElementTypes[summon.selfElement].name, locale) +
      intl.translate(summonTypes[summon.selfSummonType], locale) +
      summon.selfSummonAmount;
  }
  ...
  if (summon.friendSummonType == "odin") {
    summonHeader +=
      intl.translate("属性攻", locale) +
      summon.friendSummonAmount +
      intl.translate("キャラ攻", locale) +
      summon.friendSummonAmount2;
  } else {
    summonHeader +=
      intl.translate(summonElementTypes[summon.friendElement].name, locale) +
      intl.translate(summonTypes[summon.friendSummonType], locale) +
      summon.friendSummonAmount;
  }
  return summonHeader;
};

module.exports.calcDamage = function(
  summedAttack,
  totalSkillCoeff,
  criticalRatio,
  enemyDefense,
  ...
  var damage =
    Math.ceil(Math.ceil(summedAttack / def) * totalSkillCoeff) * criticalRatio;
  var overedDamage = 0;
...
module.exports.calcOugiDamage = function(
  summedAttack,
  totalSkillCoeff,
  criticalRatio,
  enemyDefense,
  ...
  var damage =
    (1.0 + ougiDamageUP) *
    ratio *
    Math.ceil(Math.ceil(summedAttack / def) * totalSkillCoeff) *
    criticalRatio;
...
module.exports.calcCriticalArray = function(
  normalCritical,
  _magnaCritical,
  normalOtherCritical,
  summon
...
module.exports.calcCriticalDeviation = function(criticalRatioArray) {
  ...
  for (var attackRatio in criticalRatioArray) {
    variance +=
      criticalRatioArray[attackRatio] *
      Math.pow(attackRatio - expectedValue, 2);
  }
...
module.exports.calcBasedOneSummon = function(summonind, prof, buff, totals) {
  ...
  for (var key in totals) {
    ...
    var magnaHaisuiCoeff =
      1.0 + 0.01 * (totals[key]["magnaHaisui"] * totalSummon["magna"]);
    var magnaKonshinCoeff =
      1.0 + 0.01 * (totals[key]["magnaKonshin"] * totalSummon["magna"]);
    var exCoeff =
      1.0 +
      0.01 * totals[key]["unknown"] * totalSummon["ranko"] +
      0.01 * totals[key]["ex"];
    var exHaisuiCoeff = 1.0 + 0.01 * totals[key]["exHaisui"];
    var normalCoeff =
      1.0 +
      0.01 * totals[key]["normal"] * totalSummon["zeus"] +
      0.01 * totals[key]["bahaAT"] +
      0.01 * totals[key]["normalOther"] +
      0.01 * totals[key]["cosmosAT"] +
      0.01 * totals[key]["omegaNormal"] +
      totalSummon["chara"] +
      buff["normal"] +
      totals[key]["normalBuff"];
    var normalHaisuiCoeff =
      1.0 +
      0.01 * totals[key]["normalHaisui"] * totalSummon["zeus"] +
      0.01 * totals[key]["normalOtherHaisui"];
    var normalKonshinCoeff =
      1.0 +
      0.01 * totals[key]["normalKonshin"] * totalSummon["zeus"] +
      0.01 * totals[key]["normalOtherKonshin"];
    // 属性(経過ターン)も最大値で計算する
    var elementCoeff =
      totals[key]["typeBonus"] +
      (totalSummon["element"] - 1.0 + totalSummon["elementTurn"] - 1.0) +
      buff["element"] +
      totals[key]["elementBuff"] +
      0.01 * totals[key]["LB"].Element;
    var otherCoeff =
      (1.0 + buff["other"]) *
      (1.0 + buff["other2"]) *
      (1.0 + totals[key]["otherBuff"]) *
      (1.0 + totals[key]["otherBuff2"]);
    ...
    var hpCoeff =
      ...
      buff["hp"] +
      totalSummon["hpBonus"] +
      0.01 * totals[key]["bahaHP"] +
      0.01 * totals[key]["omegaNormalHP"] +
      0.01 * totals[key]["magnaHP"] * totalSummon["magna"] +
      0.01 * totals[key]["normalHP"] * totalSummon["zeus"] +
      0.01 * totals[key]["unknownHP"] * totalSummon["ranko"] +
      0.01 * totals[key]["exHP"];
    ...
    // ベースHP
    var displayHP =
      totals[key]["baseHP"] + totals[key]["armHP"] + totalSummon["hp"];

    if (key == "Djeeta") {
      // for Djeeta
      var summedAttack =
        (totals[key]["baseAttack"] +
          totals[key]["armAttack"] +
          totalSummon["attack"] +
          totals["Djeeta"]["job"].atBonus) *
        (1.0 + buff["master"]);
      // 主人公HP計算
    ...
    var normalSante =
      totals[key]["normalSante"] * totalSummon["zeus"] +
      totals[key]["normalOtherSante"];
    ...
    // DATA debuff は羅刹用
    var armDAupNormal =
      normalNite + normalSante > 50.0 ? 50.0 : normalNite + normalSante;
    var armDAupMagna =
      magnaNite + magnaSante > 50.0 ? 50.0 : magnaNite + magnaSante;
    var armDAupBaha =
      totals[key]["bahaDA"] > 50.0 ? 50.0 : totals[key]["bahaDA"];
    var armDAupCosmos =
      totals[key]["cosmosBL"] > 50.0 ? 50.0 : totals[key]["cosmosBL"];
    var armDAupOther =
      totals[key]["DAbuff"] > 50.0 ? 50.0 : totals[key]["DAbuff"]; // 特殊スキルなどの分
    // unknownは現状50%に届くことはない
    var totalDA =
      0.01 * (totals[key]["baseDA"] + totals[key]["LB"]["DA"]) +
      buff["da"] +
      totals[key]["DABuff"] +
      totalSummon["da"] +
      0.01 *
    ...
    var normalLesserSante =
      totals[key]["normalLesserSante"] * totalSummon["zeus"];
    var magnaLesserSante =
      totals[key]["magnaLesserSante"] * totalSummon["magna"];
    var armTAupNormal =
      normalSante + normalLesserSante > 50.0
        ? 50.0
        : normalSante + normalLesserSante;
    var armTAupMagna = magnaSante > 50.0 ? 50.0 : magnaSante + magnaLesserSante;
    var armTAupBaha =
      totals[key]["bahaTA"] > 50.0 ? 50.0 : totals[key]["bahaTA"];
    var armTAupOther =
      totals[key]["TAbuff"] > 50.0 ? 50.0 : totals[key]["TAbuff"];
    var totalTA =
      0.01 * (totals[key]["baseTA"] + totals[key]["LB"]["TA"]) +
      buff["ta"] +
      totals[key]["TABuff"] +
      totalSummon["ta"] +
      0.01 * (armTAupNormal + armTAupMagna + armTAupBaha + armTAupOther);
    ...
    if (totals[key]["typeBonus"] == 1.5) {
      ...
      var damageUP =
        totals[key]["tenshiDamageUP"] > totals[key]["charaDamageUP"]
          ? totals[key]["tenshiDamageUP"]
          : totals[key]["charaDamageUP"];

      // 通常別枠の技巧スキル配列を生成
      var LBCriticalArray = getLBCriticalArray(totals[key]["LB"]);
      var normalOtherCriticalBuffArray = totals[key]["normalOtherCriticalBuff"];
      var normalOtherCriticalArray = totals[key]["normalOtherCritical"].concat(
        LBCriticalArray,
        normalOtherCriticalBuffArray
      );

      var criticalArray = module.exports.calcCriticalArray(
        totals[key]["normalCritical"],
        totals[key]["magnaCritical"],
        normalOtherCriticalArray,
        totalSummon
      );
    ...
    var expectedOugiGage =
      (buff["ougiGage"] +
        totals[key]["ougiGageBuff"] -
        totals[key]["ougiDebuff"]) *
      (taRate * 37.0 +
        (1.0 - taRate) * (daRate * 22.0 + (1.0 - daRate) * 10.0));

    var minimumTurn = Math.ceil(
      100.0 /
        (37.0 *
          (buff["ougiGage"] +
            totals[key]["ougiGageBuff"] -
    ...
    var expectedTurn =
      100.0 / expectedOugiGage > minimumTurn
        ? 100.0 / expectedOugiGage
        : minimumTurn;

    // "additionalDamage"はノーマル枠として神石効果を考慮
    var additionalDamage =
      0.01 * totals[key]["additionalDamage"] * totalSummon["zeus"] +
      totals[key]["additionalDamageBuff"] +
      buff["additionalDamage"];
    ...
    // 奥義ダメージのスキル分上限は30%?
    var ougiDamageLimitByExceed =
      totals[key]["ougiDamageLimit"] > 0.3
        ? 0.3
        : totals[key]["ougiDamageLimit"];
    var ougiDamageLimitByMagna =
      totals[key]["magnaOugiDamageLimit"] * totalSummon["magna"] > 0.3
        ? 0.3
        : totals[key]["magnaOugiDamageLimit"] * totalSummon["magna"];
    var ougiDamageLimit =
      buff["ougiDamageLimit"] +
      totals[key]["ougiDamageLimitBuff"] +
      ougiDamageLimitByMagna +
      ougiDamageLimitByExceed;

    // damageは追加ダメージなしの単攻撃ダメージ(減衰・技巧補正あり)
    var damage = module.exports.calcDamage(
      summedAttack,
      totalSkillCoeff,
      criticalRatio,
      prof.enemyDefense,
    ...
    var damageWithoutCritical = module.exports.calcDamage(
      summedAttack,
      totalSkillCoeff,
      1.0,
      prof.enemyDefense,
    ...
    // 総合攻撃力 * 技巧期待値 * 連撃期待値
    var sougou_kaisuu_gikou = parseInt(
      totalAttack * criticalRatio * expectedAttack
    );

    // 奥義ダメージ = 倍率 * (1 + 奥義ダメージバフ枠) * (1 + 奥義ダメージ上昇スキル枠)
    // 処理共通化のため係数部分(100% + deltaのdelta)のみ保存
    var ougiDamageUP =
      (1.0 + totals[key]["ougiDamageBuff"]) *
        (1.0 +
          0.01 * totals[key]["magnaOugiDamage"] * totalSummon["magna"] +
          0.01 * totals[key]["normalOugiDamage"] * totalSummon["zeus"]) -
      1.0;
    var ougiDamage = module.exports.calcOugiDamage(
      summedAttack,
      totalSkillCoeff,
      criticalRatio,
      prof.enemyDefense,
    ...
    var chainBurst = module.exports.calcChainBurst(
      buff["chainNumber"] * ougiDamage,
      buff["chainNumber"],
      module.exports.getTypeBonus(totals[key].element, prof.enemyElement)
    );

    var expectedCycleDamage =
      expectedTurn * expectedAttack * damage + // 通常攻撃 * n回
      ougiDamage +
      chainBurst / buff["chainNumber"]; // 奥義 + チェインバースト (buff["chainNumber"] は 1 以上なので除算OK)
...
module.exports.calcHaisuiValue = function(
  haisuiType,
  haisuiAmount,
  haisuiSLv,
  haisuiRemainHP
  ...
  if (
    haisuiType == "normalHaisui" ||
    haisuiType == "magnaHaisui" ||
    haisuiType == "exHaisui" ||
    haisuiType == "charaHaisui"
  ) {
    ...
    haisuiType === "normalKonshin" ||
    haisuiType === "normalOtherKonshin"
  ) {
    if (remainHP >= 0.25) {
      if (haisuiAmount === "S") {
        return 0.0;
      ...
      for (var i = 0; i < 2; i++) {
        ...
        switch (support.type) {
          case "taiyou_sinkou":
            ...
            charaHaisuiValue +=
              0.01 *
              module.exports.calcHaisuiValue("charaHaisui", "L", 10, remainHP);
            continue;
          default:
            break;
        }
...
module.exports.getTotalBuff = function(prof) {
  ...
  if (!isNaN(prof.masterBonus))
    totalBuff["master"] += 0.01 * parseInt(prof.masterBonus);
  if (!isNaN(prof.masterBonusHP))
    totalBuff["masterHP"] += 0.01 * parseInt(prof.masterBonusHP);
  ...
  if (!isNaN(prof.otherBuff2))
    totalBuff["other2"] += 0.01 * parseInt(prof.otherBuff2);
  if (!isNaN(prof.additionalDamageBuff))
    totalBuff["additionalDamage"] += 0.01 * parseInt(prof.additionalDamageBuff);
  if (!isNaN(prof.ougiGageBuff))
    totalBuff["ougiGage"] += 0.01 * parseInt(prof.ougiGageBuff);
  if (!isNaN(prof.chainNumber))
    totalBuff["chainNumber"] = parseInt(prof.chainNumber);
  if (!isNaN(prof.damageLimitBuff))
    totalBuff["damageLimit"] = 0.01 * parseFloat(prof.damageLimitBuff);
  if (!isNaN(prof.ougiDamageLimitBuff))
    totalBuff["ougiDamageLimit"] = 0.01 * parseFloat(prof.ougiDamageLimitBuff);
        ...
        if (skillname != "non") {
          ...
          if (stype == "bahaAT") {
            if (!isBahaAtIncluded) {
              // バハ短剣など
              if (
                totals[key]["race"] === "unknown" ||
                totals[key]["race"] === "seisho"
              ) {
                totals[key]["bahaAT"] +=
                  comb[i] * skillAmounts["bahaAT"][amount][slv - 1];
                isBahaAtIncluded = true;
              } else {
                var bahatype = skillname.split("-");
                if (
                  bahamutRelation[bahatype[1]]["type1"] == totals[key]["race"]
                ) {
                  totals[key]["bahaAT"] +=
                    comb[i] * skillAmounts["bahaAT"][amount][slv - 1];
                  isBahaAtIncluded = true;
            ...
            if (!isBahaAthpIncluded) {
              ...
              if (
                ...
                totals[key]["bahaAT"] +=
                  comb[i] * skillAmounts["bahaAT"][amount][slv - 1];
                totals[key]["bahaHP"] +=
                  comb[i] * skillAmounts["bahaHP"][amount][slv - 1];
                isBahaAthpIncluded = true;
              } else {
                var bahatype = skillname.split("-");
                if (
                  bahamutRelation[bahatype[1]]["type1"] ==
                    totals[key]["race"] ||
            ...
            if (
              ...
              totals[key]["bahaAT"] +=
                comb[i] * skillAmounts["bahaFUATHP"]["AT"][slv - 1];
              totals[key]["bahaHP"] +=
                comb[i] * skillAmounts["bahaFUATHP"]["HP"][slv - 1];
            } else {
              var bahatype = skillname.split("-");
              if (
                bahamutFURelation[bahatype[1]]["type1"] ==
                  totals[key]["race"] ||
            ...
            if (
              ...
              totals[key]["bahaHP"] +=
                comb[i] * skillAmounts["bahaFUHP"]["HP"][slv - 1];
              totals[key]["bahaDA"] +=
                comb[i] * skillAmounts["bahaFUHP"]["DA"][slv - 1];
              totals[key]["bahaTA"] +=
                comb[i] * skillAmounts["bahaFUHP"]["TA"][slv - 1];
            } else {
              var bahatype = skillname.split("-");
              if (
                bahamutFURelation[bahatype[1]]["type1"] ==
            ...
            if (skillname == "cosmosAT" && totals[key]["type"] == "attack") {
              ...
              totals[key]["HPdebuff"] += comb[i] * 0.4;
            } else if (
              skillname == "cosmosDF" &&
              totals[key]["type"] == "defense"
            ) {
              totals[key]["HPdebuff"] -= comb[i] * 0.1;
            } else if (
              skillname == "cosmosBL" &&
              totals[key]["type"] == "balance"
            ) {
              totals[key]["cosmosBL"] = comb[i] * 20.0;
            } else if (
              skillname == "cosmosPC" &&
              totals[key]["type"] == "pecu"
            ) {
            ...
            if (
              ...
              if (!isOmegaIncluded["omegaBase"]) {
                totals[key]["omegaNormal"] +=
                  skillAmounts["omega"]["rawATK"][slv - 1];
                totals[key]["omegaNormalHP"] +=
                  skillAmounts["omega"]["rawHP"][slv - 1];
              ...
              if (!isOmegaIncluded[omegaType]) {
                if (omegaType === "senni") {
                  totals[key]["omegaNormal"] +=
                    skillAmounts["omega"][amount][slv - 1];
                  ...
                  totals[key][
                    "normalOtherKonshin"
                  ] += module.exports.calcHaisuiValue(
                    "omegaKonshin",
                    amount,
                    slv,
                    totals[key]["remainHP"]
            ...
            if (isHaisuiType(stype)) {
              ...
              if (amount == "S") {
                // 神威小は攻撃力とHPの上昇量が等しい
                totals[key]["normal"] +=
                  comb[i] * skillAmounts["normal"][amount][slv - 1];
                totals[key]["normalHP"] +=
                  comb[i] * skillAmounts["normal"][amount][slv - 1];
              } else {
                // 神威中は攻刃(中) & 守護(中)
                totals[key]["normal"] +=
                  comb[i] * skillAmounts["normal"][amount][slv - 1];
                totals[key]["normalHP"] +=
                  comb[i] * skillAmounts["normalHP"][amount][slv - 1];
              }
            } else if (stype == "magnaKamui") {
              // 神威は攻撃力とHPの上昇量が等しい
              totals[key]["magna"] +=
                comb[i] * skillAmounts["magna"][amount][slv - 1];
              totals[key]["magnaHP"] +=
                comb[i] * skillAmounts["magna"][amount][slv - 1];
              ...
              totals[key]["magnaCritical"] +=
                comb[i] * skillAmounts["magnaCritical"][amount][slv - 1];
              totals[key]["magna"] +=
                comb[i] * skillAmounts["magna"][amount][slv - 1];
              ...
              totals[key]["normalNite"] +=
                comb[i] * skillAmounts["normalNite"][amount][slv - 1];
            } else if (stype == "magnaKatsumi") {
              totals[key]["magnaCritical"] +=
                comb[i] * skillAmounts["magnaCritical"][amount][slv - 1];
              totals[key]["magnaNite"] +=
                comb[i] * skillAmounts["magnaNite"][amount][slv - 1];
              ...
              totals[key]["normal"] +=
                comb[i] * skillAmounts["normal"][amount][slv - 1];
              totals[key]["normalLesserSante"] +=
                comb[i] * skillAmounts["normalRanbu"][amount][slv - 1];
              ...
              totals[key]["magnaLesserSante"] +=
                comb[i] * skillAmounts["magnaHakai"][amount][slv - 1];
            } else if (stype == "magnaRanbu") {
              totals[key]["magna"] +=
                comb[i] * skillAmounts["magna"][amount][slv - 1];
              totals[key]["magnaLesserSante"] +=
                comb[i] * skillAmounts["magnaRanbu"][amount][slv - 1];
            } else if (stype == "magnaGunshin") {
              totals[key]["magnaHP"] +=
                comb[i] * skillAmounts["magnaHP"][amount][slv - 1];
              totals[key]["magnaNite"] +=
                comb[i] * skillAmounts["magnaNite"][amount][slv - 1];
            } else if (stype == "normalHiou") {
              totals[key]["normalOugiDamage"] +=
                comb[i] * skillAmounts["normalHiou"][amount][slv - 1];
            } else if (stype == "magnaHissatsu") {
              totals[key]["magnaOugiDamage"] +=
                comb[i] * skillAmounts["magnaHissatsu"][amount][slv - 1];
              totals[key]["magnaOugiDamageLimit"] +=
                0.01 * comb[i] * skillAmounts["magnaHissatsu"][amount][slv - 1];
            } else if (stype == "exBoukun") {
              totals[key]["HPdebuff"] += comb[i] * 0.07;
              totals[key]["ex"] +=
                comb[i] * skillAmounts["ex"][amount][slv - 1];
            } else if (stype == "exATKandHP") {
              totals[key]["ex"] +=
                comb[i] * skillAmounts["exATKandHP"][amount]["ATK"][slv - 1];
              totals[key]["exHP"] +=
                comb[i] * skillAmounts["exATKandHP"][amount]["HP"][slv - 1];
              ...
              if (index == 4) {
                ...
                totals[key]["ougiDebuff"] += comb[i] * 0.3;
              }
              //! 四大天司の祝福
            } else if (stype == "tenshiShukufuku") {
              if (amount == "M") {
                totals[key]["tenshiDamageUP"] += comb[i] * 0.1;
              } else if (amount == "L") {
                totals[key]["tenshiDamageUP"] += comb[i] * 0.2;
              }
              //! ダメージ上限アップ系は奥義と通常どちらにも効く
            } else if (stype == "normalDamageLimit") {
              totals[key]["normalDamageLimit"] +=
                comb[i] * skillAmounts["normalDamageLimit"][amount];
              totals[key]["ougiDamageLimit"] +=
                comb[i] * skillAmounts["normalDamageLimit"][amount];
            } else if (stype == "ougiDamageLimit") {
              totals[key]["ougiDamageLimit"] +=
                comb[i] * skillAmounts["ougiDamageLimit"][amount];
            } else if (stype == "ougiDamageLimitExceed") {
              totals[key]["ougiDamageLimit"] +=
                0.01 *
                comb[i] *
                skillAmounts["ougiDamageLimitExceed"][amount][slv - 1];
              //! 4凸武器スキル
              ...
              if (key == "Djeeta")
                totals[key]["DAbuff"] +=
                  comb[i] * skillAmounts["washiouKekkai"][amount][slv - 1];
            } else if (stype == "maihimeEnbu") {
              // 舞姫の演武: 通常攻刃大 + 上限アップ7%
              totals[key]["normal"] +=
                comb[i] * skillAmounts["normal"]["L"][slv - 1];
              totals[key]["normalDamageLimit"] +=
                comb[i] * skillAmounts["normalDamageLimit"]["M"];
              totals[key]["ougiDamageLimit"] +=
                comb[i] * skillAmounts["normalDamageLimit"]["M"];
              // メインのみDATA追加拡張
              ...
              totals[key][stype] +=
                comb[i] * skillAmounts[stype][amount][slv - 1];
            }
          }
        }
      }
  ...
  LBCriticalKeys.forEach(crit_key => {
    if (charaLB[crit_key] !== "none") {
      ...
      criticalArray.push({
        ...
        attackRatio:
          GlobalConst.limitBonusCriticalList[chara_lb_crit_type].attackRatio
      });
    }
  });
...
module.exports.getInitialTotals = function(prof, chara, summon) {
  ...
  var djeetaRemainHP =
    prof.remainHP != undefined && parseInt(prof.remainHP) < parseInt(prof.hp)
      ? 0.01 * parseInt(prof.remainHP)
      : 0.01 * parseInt(prof.hp);
  ...
  var zenithATK =
    prof.zenithAttackBonus == undefined
      ? 3000
      : parseInt(prof.zenithAttackBonus);
  var zenithHP =
    prof.zenithHPBonus == undefined ? 1000 : parseInt(prof.zenithHPBonus);
  var zenithPartyHP =
    prof.zenithPartyHPBonus == undefined
      ? 0
      : parseInt(prof.zenithPartyHPBonus);
  ...
  for (var i = 0; i < chara.length; i++) {
    if (chara[i].name != "") {
      var charaelement =
        chara[i].element == undefined ? "fire" : chara[i].element;
      var charaDA = chara[i].DA == undefined ? 6.5 : chara[i].DA;
      var charaTA = chara[i].TA == undefined ? 3.0 : chara[i].TA;
      var charaRemainHP =
        chara[i].remainHP != undefined &&
        parseInt(chara[i].remainHP) < parseInt(prof.hp)
          ? 0.01 * parseInt(chara[i].remainHP)
          : 0.01 * parseInt(prof.hp);
      var charaConsidered =
        chara[i].isConsideredInAverage == undefined
          ? true
          : chara[i].isConsideredInAverage;
      ...
      for (var charabuffkey in charaBuffList) {
        if (chara[i][charabuffkey] != undefined) {
          charaBuffList[charabuffkey] =
            0.01 * parseFloat(chara[i][charabuffkey]);
        }
      }
  ...
  for (var key in totals) {
    ...
    for (var s = 0; s < summon.length; s++) {
      var selfElement =
        summon[s].selfElement == undefined ? "fire" : summon[s].selfElement;
      var friendElement =
        summon[s].friendElement == undefined ? "fire" : summon[s].friendElement;
      ...
      if (
        summonElementTypes[selfElement]["type"].indexOf(
          totals[key]["element"]
        ) >= 0 ||
        selfElement == "all"
      ) {
        if (summon[s].selfSummonType == "odin") {
          ...
          totalSummon["element"] +=
            0.01 *
            module.exports.getTesukatoripokaAmount(
              parseInt(summon[s].selfSummonAmount),
              races
            );
        } else {
          // 自分の加護 通常の場合
          totalSummon[summon[s].selfSummonType] +=
            0.01 * parseInt(summon[s].selfSummonAmount);
        }
      }
      if (
        summonElementTypes[friendElement]["type"].indexOf(
          totals[key]["element"]
        ) >= 0 ||
        friendElement == "all"
      ) {
        if (summon[s].friendSummonType == "odin") {
          // odin(属性+キャラ攻撃)など、複数の場合の処理
          totalSummon["element"] +=
            0.01 * parseInt(summon[s].friendSummonAmount);
          totalSummon["chara"] +=
            0.01 * parseInt(summon[s].friendSummonAmount2);
        } else if (summon[s].friendSummonType == "elementByRace") {
          totalSummon["element"] +=
            0.01 *
            module.exports.getTesukatoripokaAmount(
              parseInt(summon[s].friendSummonAmount),
              races
            );
        } else {
          // フレンドの加護 通常の場合
          totalSummon[summon[s].friendSummonType] +=
            0.01 * parseInt(summon[s].friendSummonAmount);
        }
      }

      // 後から追加したので NaN でないか判定しておく
      if (!isNaN(summon[s].attack))
        totalSummon["attack"] = parseInt(summon[s].attack);
      if (!isNaN(summon[s].hp)) totalSummon["hp"] = parseInt(summon[s].hp);
      if (!isNaN(summon[s].hpBonus))
        totalSummon["hpBonus"] = 0.01 * parseInt(summon[s].hpBonus);
      if (!isNaN(summon[s].DA))
        totalSummon["da"] = 0.01 * parseInt(summon[s].DA);
      if (!isNaN(summon[s].TA))
        totalSummon["ta"] = 0.01 * parseInt(summon[s].TA);
  ...
  for (var key in totals) {
    totals[key]["typeBonus"] = module.exports.getTypeBonus(
      totals[key]["element"],
      prof.enemyElement
    );
  }
...
module.exports.calcOneCombination = function(
  comb,
  summon,
  prof,
  arml,
    ...
    for (var i = 0; i < 2; i++) {
      ...
      switch (support.type) {
        ...
        case "normalBuff_depends_races":
          ...
          totals[key]["normalBuff"] += races == 4 ? 0.5 : races * 0.1;
          continue;
        case "normalBuff_depends_member":
          continue;
        case "taiyou_sinkou":
          // ザルハメリナのHPを参照する
          var charaHaisuiValue = module.exports.calcHaisuiValue(
            "charaHaisui",
            "L",
            10,
            totals[key]["remainHP"]
...
module.exports.generateHaisuiData = function(
  res,
  arml,
  summon,
  prof,
  ...
  for (var ch = 0; ch < chara.length; ch++) {
    var charaConsidered =
      chara[ch].isConsideredInAverage == undefined
        ? true
        : chara[ch].isConsideredInAverage;
  ...
  var charaHaisuiBuff = [];
  for (var k = 0; k < 100; ++k) {
    var charaHaisuiValue = module.exports.recalcCharaHaisui(
      chara,
      0.01 * (k + 1)
    );
  ...
  for (var s = 0; s < res.length; s++) {
    ...
    for (var j = 0; j < oneresult.length; j++) {
      ...
      for (var key in onedata) {
          ...
          for (var jj = 1; jj <= 2; jj++) {
            ...
            if (skillname != "non") {
              ...
              if (skillname === "omega-gekijou") {
                if (
                  !omegaHaisuiIncluded &&
                  (arm.armType === onedata[key].fav1 ||
                    arm.armType === onedata[key].fav2)
                ) {
                  for (var l = 0; l < haisuiBuff.length; l++) {
                    var remainHP = 0.01 * (l + 1);
                    haisuiBuff[l]["normalHaisui"] +=
                      0.01 *
                      module.exports.calcHaisuiValue(
                        "normalHaisui",
                        amount,
                ...
                if (
                  !omegaKonshinIncluded &&
                  (arm.armType === onedata[key].fav1 ||
                    arm.armType === onedata[key].fav2)
                ) {
                  for (var l = 0; l < haisuiBuff.length; l++) {
                    var remainHP = 0.01 * (l + 1);
                    haisuiBuff[l]["normalKonshin"] +=
                      0.01 *
                      module.exports.calcHaisuiValue(
                        "omegaKonshin",
                        amount,
        ...
        for (var k = 0; k < 100; k++) {
          var newTotalSkillCoeff =
            totalSkillWithoutHaisui *
            haisuiBuff[k].normalHaisui *
            haisuiBuff[k].magnaHaisui *
            haisuiBuff[k].normalKonshin *
            haisuiBuff[k].magnaKonshin *
            haisuiBuff[k].charaHaisui *
            haisuiBuff[k].exHaisui;
          var summedAttack = onedata[key].displayAttack;
          var newTotalAttack = summedAttack * newTotalSkillCoeff;
          var newTotalExpected =
            newTotalAttack *
            onedata[key].criticalRatio *
            onedata[key].expectedAttack;

          var newDamage = module.exports.calcDamage(
            summedAttack,
            newTotalSkillCoeff,
            onedata[key].criticalRatio,
            prof.enemyDefense,
          ...
          var newOugiDamage = module.exports.calcOugiDamage(
            summedAttack,
            newTotalSkillCoeff,
            onedata[key].criticalRatio,
            prof.enemyDefense,
          ...
          var chainNumber = isNaN(prof.chainNumber)
            ? 1
            : parseInt(prof.chainNumber);
          var newChainBurst =
            module.exports.calcChainBurst(
              chainNumber * newOugiDamage,
              chainNumber,
              module.exports.getTypeBonus(
                onedata[key].element,
                prof.enemyElement
              )
            ) / chainNumber;
          var newExpectedCycleDamagePerTurn =
            (newChainBurst +
              newOugiDamage +
              onedata[key].expectedTurn *
                onedata[key].expectedAttack *
                newDamage) /
            (onedata[key].expectedTurn + 1);
          ...
          if (key == "Djeeta") {
            ...
            if (hp in alreadyUsedHP) {
              ...
              if (res.length > 1) {
                ...
                if (hp in allAlreadyUsedHP) {
                  ...
                  for (
                    var subj = 0;
                    subj < res.length * oneresult.length;
                    subj++
                  ) {
            ...
            TotalHP[index][j + 1] = displayRealHP
              ? hp
              : parseInt(hp * onedata[key].totalHP);
            TotalAttack[index][j + 1] = parseInt(newTotalAttack);
            TotalExpected[index][j + 1] = parseInt(newTotalExpected);
            CriticalAttack[index][j + 1] = parseInt(
              onedata[key].criticalRatio * newTotalAttack
            );
            CycleDamagePerTurn[index][j + 1] = parseInt(
              newExpectedCycleDamagePerTurn
            );
            AverageTotalAttack[index][j + 1] += parseInt(newTotalAttack / cnt);
            AverageTotalExpected[index][j + 1] += parseInt(
              newTotalExpected / cnt
            );
            AverageCycleDamagePerTurn[index][j + 1] += parseInt(
              newExpectedCycleDamagePerTurn / cnt
            );
            AverageCriticalAttack[index][j + 1] += parseInt(
              onedata[key].criticalRatio * newTotalAttack / cnt
            );
      ...
      if (res.length > 1) {
        for (var k = 0; k < 100; k++) {
          ...
          AllCycleDamagePerTurn[allindex][allj] =
            CycleDamagePerTurn[index][j + 1];
          AllAverageTotalExpected[allindex][allj] =
            AverageTotalExpected[index][j + 1];
          AllAverageTotalAttack[allindex][allj] =
            AverageTotalAttack[index][j + 1];
          AllAverageCriticalAttack[allindex][allj] =
            AverageCriticalAttack[index][j + 1];
          AllAverageCycleDamagePerTurn[allindex][allj] =
            AverageCycleDamagePerTurn[index][j + 1];
        }
      }
    }
    ...
    for (var summonkey in data) {
      for (var k = 1; k <= 100; k++) {
        for (var j = 1; j <= res[0].length; j++) {
          // グラフ最大値最小値を保存
          if (data[summonkey][key][k][j] > minMaxArr[key]["max"])
            minMaxArr[key]["max"] = data[summonkey][key][k][j];
          if (
            data[summonkey][key][k][j] < minMaxArr[key]["min"] ||
            minMaxArr[key]["min"] == 0
          )
            minMaxArr[key]["min"] = data[summonkey][key][k][j];
        }
      }
    }
  }
...
module.exports.generateSimulationData = function(
  res,
  turnBuff,
  arml,
  summon,
  ...
  for (var s = 0; s < res.length; s++) {
    ...
    for (var t = 1; t <= turnBuff.maxTurn; t++) {
      ...
      for (var j = 0; j < turndata.length; j++) {
        ...
        for (var key in onedata) {
          if (
            turnBuff.buffs["全体バフ"][t - 1].turnType == "ougi" ||
            turnBuff.buffs[key][t - 1].turnType == "ougi"
          ) {
            // 基本的に奥義の設定が優先
            var newOugiDamage = module.exports.calcOugiDamage(
            ...
            turnBuff.buffs["全体バフ"][t - 1].turnType == "ougiNoDamage" ||
            turnBuff.buffs[key][t - 1].turnType == "ougiNoDamage"
          ) {
            // しコルワ
            if (key == "Djeeta") {
            ...
            if (key == "Djeeta") {
              ExpectedDamage[t].push(
                parseInt(newDamage * onedata[key].expectedAttack)
              );
              AverageExpectedDamage[t][j + 1] += parseInt(
                onedata[key].expectedAttack * newDamage / cnt
              );
            } else if (considerAverageArray[key]) {
              AverageExpectedDamage[t][j + 1] += parseInt(
                onedata[key].expectedAttack * newDamage / cnt
        ...
        if (t == 1) {
          ...
          if (res.length > 1) {
            ...
            AllSummedAverageExpectedDamage[0].push(
              "[" + summonHeader + "] " + title
            );
          }
          SummedAverageExpectedDamage[t][j + 1] =
            AverageExpectedDamage[t][j + 1];
        } else {
          SummedAverageExpectedDamage[t][j + 1] =
            SummedAverageExpectedDamage[t - 1][j + 1] +
            AverageExpectedDamage[t][j + 1];
        }
        ...
        if (res.length > 1) {
          ...
          AllSummedAverageExpectedDamage[t].push(
            SummedAverageExpectedDamage[t][j + 1]
          );
        }
      }
  ...
  if (res.length > 1) {
    ...
    data[matomete][
      "summedAverageExpectedDamage"
    ] = AllSummedAverageExpectedDamage;
  }
